/*Code for September, 2020. Completed 1/15/21. Jessica Song*/
filename Wider 
		'C:\Users\16147\OneDrive - Kent State University\Kent State\Active Projects\BroadStreet_COVID\For SAS\September_Wide.xlsx';
proc import datafile=Wider 
out= Wider
dbms=xlsx
replace;
getnames=yes;
run; /*Only 30 days. Adjust code below*/
...
proc contents data=wider; run; /*Check for character variables*/
...
by County_Name;
var White_Count White_Count_1-White_Count_29;
...
data long_Aug;
   ...
   drop _name_;
	...
	proc contents data=long_aug; run;/*Asian, Native, Pacific, NonHispanic, Pacific, Unspec.*/

data long_aug1; set long_aug;
      Asian_Count1 = input(Asian_Count, 8.);
   drop Asian_Count;
   rename Asian_Count1=Asian_Count;
      Native_Count1 = input(Native_Count, 8.);
   drop Native_Count;
   rename Native_Count1=Native_Count;
      Pacific_Count1 = input(Pacific_Count, 8.);
   drop Pacific_Count;
   rename Pacific_Count1=Pacific_Count;
      Other_Count1 = input(Other_Count, 8.);
   drop Other_Count;
   rename Other_Count1=Other_Count;
...
PROC EXPORT DATA= WORK.final
            OUTFILE= 'C:\Users\16147\OneDrive - Kent State University\Kent State\Active Projects\BroadStreet_COVID\Final Products\September.xlsx'
            DBMS=EXCEL REPLACE; 
RUN;
