/*Code for July, 2020. Completed 1/15/21. Jessica Song*/
filename Wider 
		'C:\Users\16147\OneDrive - Kent State University\Kent State\Active Projects\BroadStreet_COVID\For SAS\July_Wide.xlsx';
...
data long_Aug;
   ...
   drop _name_;
	...
	proc contents data=long_aug; run;/*All are character variables which will be an issue. Need to resolve.*/

data long_aug1; set long_aug;
   White_Count1 = input(White_Count, 8.);
   drop White_Count;
   rename White_Count1=White_Count;
   Black_Count1 = input(Black_Count, 8.);
   drop Black_Count;
   rename Black_Count1=Black_Count;
   ...
   rename Pacific_Count1=Pacific_Count;
      TwoPlus_Count1 = input(TwoPlus_Count, 8.);
   drop TwoPlus_Count;
   rename TwoPlus_Count1=TwoPlus_Count;
      Other_Count1 = input(Other_Count, 8.);
   drop Other_Count;
   rename Other_Count1=Other_Count;
      Unk_Count1 = input(Unk_Count, 8.);
   drop Unk_Count;
   rename Unk_Count1=Unk_Count;
     Hispanic_Count1 = input(Hispanic_Count, 8.);
   drop Hispanic_Count;
   rename Hispanic_Count1=Hispanic_Count;
...
PROC EXPORT DATA= WORK.final
            OUTFILE= 'C:\Users\16147\OneDrive - Kent State University\Kent State\Active Projects\BroadStreet_COVID\Final Products\July.xlsx'
            DBMS=EXCEL REPLACE; 
RUN;
